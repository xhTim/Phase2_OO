...
void readFiles( )
{
	for(int is=0; is<nSpecs; is++)
	{
		...
		{
			f[is] = TFile::Open( Form("outFiles/ana_mc_%s_ZB.root", specFileName[is].Data()) );
		}
		else
		{
			f[is] = TFile::Open( Form("outFiles/ana_mc_%s_ZB.root", specFileName[is].Data()) );
...
void saveFiles( )
{
	...
	*/
	...
	//----------------------------------------------------------------------------------------------------------------------------------------------------------
	TFile *fOutTemp = new TFile( Form("%s/MassPtTemp_AllSpecs"+massname+"%dRapBins%s%s_ZB.root", outDir[template_option].Data(), nDiffRapBins, TnPcases[RunTnPcase].Data(), HFcases[RunHFcase].Data()), "recreate");
