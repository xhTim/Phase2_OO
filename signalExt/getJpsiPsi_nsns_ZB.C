...
#include "../common_ZB/funUtil.h"
#include "../common_ZB/LoadSignal.C"
#include "../common_ZB/PdfFactory.C"
#include "../common_ZB/HistWorker.h"
...
void prepareData()
{
	TString inFileDir 		  = Form("../anaSimu/outFiles/rawSig%s_ZB.root", HFcases[RunHFcase].Data());
...
void loadEff()
{
	LoadEfficiency Efficiency("../anaSimu/EffundTemp/Eff_CohJpsi_ZB.root", SymmetricRapBin);
...
void loadAcc()
{
	LoadAcceptance Acceptance(Form("../simulation/out4AccFactors/Acceptance_AllSpecs_%dRapBins_ZB.root", nDiffRapBins), SymmetricRapBin);
...
void fitCohMass_4RNRfD( const double massLow4Fit=2.6, const double massHig4Fit=4.2)
{
	...
	TFile *inf_Temps = TFile::Open(Form("../anaSimu/out4effAndTemp/MassPtTemp_AllSpecs_massWindow_2.95_3.25_%dRapBins%s_ZB.root", nDiffRapBins, TnPcases[RunTnPcase].Data()));
	TF1 *fQED;
...
//			double eff_CohPsi2Jpsi  = H_EffVsY_CohPsi2Jpsi ->GetBinContent(H_EffVsY_CohPsi2Jpsi->FindBin(yMean));
			...
			double acc_CohJpsi 		= 1.0; //H_AccVsY_CohJpsi	   ->GetBinContent(H_AccVsY_CohJpsi    ->FindBin(yMean));
...
//			cout<<"eff_CohPsi2Jpsi: "<<eff_CohPsi2Jpsi<<endl;
			...
			RooRealVar  jpsiSigma	( "jpsiSigma",   	"jpsiSigma",	0.045,	0.01,	0.3 	);
...
//			drawLatex(.45, 0.25+textDy, Form("f_{D} = #frac{FD J/#psi}{primary J/#psi} = %.3f #pm %.4f",      fD,fDErr),  mTextFont, mTextSize, mTextColor);
			...
			if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins){
				c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm.png",  iy) );
				c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm.pdf",  iy) );
			}
			else{
				c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d.png",  iy) );
				c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d.pdf",  iy) );			
			}
			//----------------------------------------------------------------------------------------------------------------------------------------------------
...
//				drawLatex(.58, 0.33+textDy, Form("f_{D}  = %.3f #pm %.3f",  fD,                           fDErr),  mTextFont, mTextSize-0.005, mTextColor);
				...
				//----------------------------------------------------------------------------------------------------------------------------------------------------
				if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins){
					c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm_4paper.png",  iy) );
					c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm_4paper.pdf",  iy) );
				}
				else{
					c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_4paper.png",  iy) );
					c1->SaveAs( Form("outplots_ZB/massSpec_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_4paper.pdf",  iy) );			
				}
				//----------------------------------------------------------------------------------------------------------------------------------------------------
			...
			//if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins){
			//c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm.png",  iy) );
			//c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d_Symm.pdf",  iy) );
			//}
			//else{
			//c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d.png",  iy) );
			//c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4JpsiPsi_"+nCasesName[i_ncase]+"_iy%d.pdf",  iy) );
			//}
...
void fitFullMassAndPt_4Decouple( const double massLow4Fit=2.6, const double massHig4Fit=4.2, const double ptLow4Fit = 0.0, const double ptHig4Fit = 3.5 )
{
	...
	TFile *inf_Temps = TFile::Open(Form("../simulation/%s/MassPtTemp_AllSpecs_massWindow_2.95_3.25_%dRapBins%s_ZB.root", template_Dir[template_option].Data(), nDiffRapBins, TnPcases[RunTnPcase].Data()));
	...
	for(int i_ncase=0; i_ncase<NnCases; i_ncase++)
	{
		...
		//	}
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------
			if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins) c1->SaveAs( Form("outplots_ZB/massSpec_4ptFitConstrain_"+nCasesName[i_ncase]+"_iy%d_Symm.pdf",  iy) );
			else c1->SaveAs( Form("outplots_ZB/massSpec_4ptFitConstrain_"+nCasesName[i_ncase]+"_iy%d.pdf",  iy) );
			//----------------------------------------------------------------------------------------------------------------------------------------------------
			...
			//----------------------------------------------------------------------------------------------------------------------------------------------------
			// if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins) 
			// 	c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4ptFitConstrain_"+nCasesName[i_ncase]+"_iy%d_Symm.png",  iy) );
			// else 
			// 	c1->SaveAs( Form("outplots_ZB/massSpecFitPull_4ptFitConstrain_"+nCasesName[i_ncase]+"_iy%d.png",  iy) );
			//----------------------------------------------------------------------------------------------------------------------------------------------------
...
//
			...
			//
			if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins){
				c2->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_Symm.png",  iy) );
				c2->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_Symm.pdf",  iy) );
			}
			else{
				c2->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d.png",  iy) );
				c2->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d.pdf",  iy) );
			}
			...
			if(draw4Paper_flag==1)
			{
				...
				if(SymmetricRapBin && iy > (nDiffRapBins/2 - 1) && iy < nDiffRapBins)
				{
					c3->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_Symm_4paper.png",  iy) );
					c3->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_Symm_4paper.pdf",  iy) );
				}
				else
				{
					c3->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_4paper.png",  iy) );
					c3->SaveAs( Form("outplots_ZB/ptSpec_4decouple_"+nCasesName[i_ncase]+"_iy%d_4paper.pdf",  iy) );
				}
...
void saveFile(const TString headerTitle = "Default")
{
	TFile *file_JpsiXsec = new TFile(Form("JpsiXsecValues/JpsiXsec_%s_%dRapBins%s%s%s_ZB.root", headerTitle.Data(), nDiffRapBins, template_Name[template_option].Data(), TnPcases[RunTnPcase].Data(), HFcases[RunHFcase].Data()), "recreate");
